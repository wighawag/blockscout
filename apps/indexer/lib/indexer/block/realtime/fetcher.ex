defmodule Indexer.Block.Realtime.Fetcher do
  ...
  alias Indexer.Block.Realtime.Fetcher.TaskSupervisor
  ...
  defp start_fetch_and_import(number, block_fetcher, previous_number) do
    ...
    for block_number_to_fetch <- start_at..number do
      ...
      options = [timeout: :timer.minutes(5)]
      Task.Supervisor.start_child(TaskSupervisor, __MODULE__, :fetch_and_import_block, args, options)
    end
  end
